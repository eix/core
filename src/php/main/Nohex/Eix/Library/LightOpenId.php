 ...
 */
/*
 * Minor modifications to keep it compatible with Eix's autoloader.
 */

namespace Nohex\Eix\Library;
...
class LightOpenId
{
    ...
    public function setHost($host)
    {
        ...
        // If the return URL is too long, the request arrays may end up not
        // populated.
        if (empty($this->data)) {
            $url = parse_url($_SERVER['REQUEST_URI']);
            parse_str(@$url['query'], $this->data);
        }
    ...
    public function discover($url)
    {
        Logger::get()->debug(sprintf(
            'OpenID validation: discovering claimed ID %s.',
            $this->claimed_id
        ));

        if (!$url) {
            Logger::get()->debug(
                'OpenID discovery failed: no URL.'
            );
                ...
                if (isset($headers['content-type'])
                    ...
                    foreach ($m[1] as $content) {
                        ...
                        if (preg_match('#<Type>\s*' . $ns . '(server|signon)\s*</Type>#s', $content, $type)) {
                            ...
                            if (empty($server)) {
                                Logger::get()->debug(
                                    'OpenID discovery failed: no server.'
                                );
    ...
    public function validate()
    {
        ...
        if ($this->mode != 'id_res') {
            Logger::get()->debug(
                'OpenID validation failed: no id_res found.'
            );
        ...
        if ($this->data['openid_return_to'] != $this->returnUrl) {
            ...
            Logger::get()->debug(sprintf(
                'OpenID validation failed: return URL (%s) does not match the current document\'s (%s).',
        ...
        Logger::get()->debug(sprintf(
            'OpenID validation: ' . ($isValid ? 'valid' : 'not valid'),
            $this->claimed_id
        ));
